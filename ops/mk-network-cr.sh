...
function NetworkCRD()
{
  ...
  for NETOP_SU in ${NETOP_SULIST[@]};do
    for NIDXDEF in ${NETOP_NETLIST[@]};do
      ...
      # sriovnet_dra pods attach VFs through ResourceClaim (generated by
      # mk-dra-cr.sh) instead of via a multus annotation pointing at a
      # SriovNetwork CR — so skip the per-app-namespace network-attachment CR.
      # SriovNetworkNodePolicy (created above) is still needed to allocate the
      # VFs on the host.
      if [ "${USECASE}" != "sriovnet_dra" ]; then
