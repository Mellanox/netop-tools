...
# switch ports.
#
# This is intentionally dry-run by default. Dry-run still backs up the current
# switch config for each planned switch, which validates switch credentials before
# any apply is attempted. Pass --apply to copy each generated
...
Options:
  ...
  --apply                Copy patches to switches and run nv config
                         patch/diff/apply. Default is dry-run.
                         Dry-run still captures current switch configs to test
                         switch credentials.
...
function switch_backup_done()
{
  local idx=${1}
  local item

  for item in "${SWITCH_BACKUP_DONE[@]}"; do
    [ "${item}" = "${idx}" ] && return 0
  done
  return 1
}

function backup_switch_config()
{
  local idx=${1}
  local switch_name="${SWITCH_CFG_NAMES[${idx}]}"
  local switch_host="${SWITCH_CFG_HOSTS[${idx}]}"
  local target
  local password
  local safe_name
  local output
  ...
  if switch_backup_done "${idx}"; then
    return 0
  fi
  ...
  safe_name=$(safe_file_component "${switch_name}")
  output="${REPORT_DIR}/switch-${safe_name}-before-apply.yaml"
  tmp="${output}.tmp"
  err="${output}.err"

  {
    ...
    echo "# Purpose: pre-change snapshot of existing NVUE switch configuration."
    echo
  } > "${tmp}"

  echo "BACKUP ${target} -> ${output}"
  if [ -n "${password}" ]; then
    if ! command -v sshpass >/dev/null 2>&1; then
    ...
    if ! SSHPASS="${password}" sshpass -e "${SWITCH_SSH_CMD[@]}" "${target}" \
      "nv config show -o yaml 2>/dev/null || nv config show --output yaml 2>/dev/null || nv config show" \
      >> "${tmp}" 2>"${err}"; then
      echo "ERROR: failed to back up switch config from ${target}; see ${err}" >&2
      rm -f "${tmp}"
      return 1
    fi
  else
    if ! "${SWITCH_SSH_CMD[@]}" "${target}" \
      "nv config show -o yaml 2>/dev/null || nv config show --output yaml 2>/dev/null || nv config show" \
      >> "${tmp}" 2>"${err}"; then
      echo "ERROR: failed to back up switch config from ${target}; see ${err}" >&2
      rm -f "${tmp}"
  ...
  mv "${tmp}" "${output}"
  printf '%s\t%s\t%s\t%s\n' "${idx}" "${switch_name}" "${switch_host}" "${output}" >> "${BACKUP_LIST}"
  SWITCH_BACKUP_DONE+=( "${idx}" )
}
...
function apply_patch_to_switch()
{
  ...
  if ! backup_switch_config "${idx}"; then
    return 1
  fi
...
BACKUP_LIST="${REPORT_DIR}/backups.txt"
: > "${PLAN}"
: > "${PATCH_LIST}"
: > "${WARNINGS}"
: > "${BACKUP_LIST}"
...
echo
echo "Capturing current switch configs..."
while IFS=$'\t' read -r switch_idx _switch_name _switch_host _port _patch_file; do
  if ! backup_switch_config "${switch_idx}"; then
    echo "ERROR: failed capturing current switch config for switch index ${switch_idx}" >&2
    exit 1
  fi
done < "${PATCH_LIST}"

{
  ...
  if [ -s "${BACKUP_LIST}" ]; then
    echo
    echo "captured switch configs:"
    awk -F'\t' '{ print "  " $4 }' "${BACKUP_LIST}"
  fi
  if [ -s "${WARNINGS}" ]; then
    echo
    echo "warnings:"
...
if [ "${APPLY}" = "true" ]; then
  ...
  while IFS=$'\t' read -r switch_idx _switch_name _switch_host _port patch_file; do
    if ! apply_patch_to_switch "${switch_idx}" "${patch_file}"; then
      echo "ERROR: failed applying ${patch_file}" >&2
      exit 1
    fi
  done < "${PATCH_LIST}"
else
