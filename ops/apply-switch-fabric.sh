...
# switch ports.
#
# This is intentionally dry-run by default. Pass --apply to back up the current
# switch config, copy each generated switch-<switch>-<port>-L3.yaml patch to the
# switch, and run nv config patch, nv config diff, and nv config apply.
#
...
Options:
  ...
  --apply                Back up current switch configs, copy patches to
                         switches, and run nv config patch/diff/apply.
                         Default is dry-run.
...
function backup_switch_config()
{
...

  {
    ...
    echo "# Purpose: pre-apply backup of existing NVUE switch configuration."
    echo
  } > "${tmp}"
...
if [ "${APPLY}" = "true" ]; then
  ...
  if [ -s "${BACKUP_LIST}" ]; then
    echo
    echo "Switch config backups:"
    awk -F'\t' '{ print "  " $4 }' "${BACKUP_LIST}" | tee -a "${SUMMARY}"
  fi
else
  echo
