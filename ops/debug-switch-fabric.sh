...
function write_switch_fix_yaml()
{
  ...
  for port in ${ports}; do
    output_file="${REPORT_DIR}/switch-$(safe_file_component "${name}")-$(safe_file_component "${port}")-vlan0-L2.yaml"
    {
      ...
      echo "#   sudo nv config patch /absolute/path/$(basename "${output_file}")"
...
function run_switch_checks()
{
  ...
  for idx in "${!SWITCH_CFG_HOSTS[@]}"; do
...

    {
      ...
      if [ -n "${fix_files}" ]; then
        echo "proposed vlan 0 L2 fix YAML:"
        printf '%s\n' "${fix_files}" | sed 's/^/  /'
      else
        echo "proposed vlan 0 L2 fix YAML: none generated; no switch ports matched this switch entry"
      fi
      if [ -n "${password}" ] || [ -n "${password_env}" ]; then
        if ! command -v sshpass >/dev/null 2>&1; then
