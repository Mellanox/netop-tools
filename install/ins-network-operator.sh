...
if [ "${X}" = "0" ];then 
  ${docmd} kubectl create ns ${NETOP_NAMESPACE}
fi
...
${docmd} kubectl apply -f ${USECASE_DIR}/NicClusterPolicy.yaml
if [ "${NIC_CONFIG_ENABLE}" = "true" ];then
  for DEVICE_TYPE in ${DEVICE_TYPES[@]};do
    ${docmd} kubectl appy -f ${USECASE_DIR}/nic-config-crd-${DEVICE_TYPE}.yaml
  done
fi
