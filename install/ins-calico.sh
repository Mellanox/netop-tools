...
if ! ${HELMCL} repo add projectcalico https://docs.tigera.io/calico/charts; then
    echo "ERROR: Failed to add Calico Helm repository"
    exit 1
fi

echo "Updating Helm repositories..."
if ! ${HELMCL} repo update; then
...
if ! ${HELMCL} install calico projectcalico/tigera-operator --version "${CALICO_VERSION}" --namespace tigera-operator; then
...
#${HELMCL} install calico projectcalico/tigera-operator --version ${CALICO_VERSION} -f values.yaml --namespace tigera-operator
...
### 
### ${HELMCL} repo add projectcalico https://docs.tigera.io/calico/charts
### # Install the Tigera Calico operator and custom resource definitions using the Helm chart:
### 
### #${HELMCL} install calico projectcalico/tigera-operator --version ${CALICO_VERSION} --namespace tigera-operator
### ${HELMCL} install calico projectcalico/tigera-operator --namespace tigera-operator
### 
### ${K8CL} apply -f ./calico/calico-custom-resources.yaml
### #${HELMCL} upgrade tigera-operator -f calico/calico-custom-resources.yaml tigera-operator
